...
MessagePack 类似于JSON的对象序列化规范.

MessagePack 有两个概念即: **type system** 和 **formats**

MessagePack序列化是指通过type system将对象转换成MessagePack formats。
...
该文档描述了MessagePack的type system,formats以及它们的转换过程。
...
* Types
  ...
  * **Float** 表示包括了NaN和无穷大的IEEE 754点双精度浮点数
  * **Raw**
      * **String** 扩展的 Raw 类型 表示一个 UTF-8 字符串
      * **Binary** 扩展的 Raw 类型 表示一个字节数组
  * **Array** 表示一系列对象
  * **Map** 表示对象的键值对
  * **Extension** 表示一个类型信息和字节数组对元组。其中类型信息是由应用定义的一个整数。

<a name="types-limitation"/>
### 取值范围

* 一个整数对象的取值范围：`-(2^63)` ~ `(2^64)-1`
* 一个二进制对象的最大长度：`(2^32)-1`
* 一个字符串对象的最大字节大小：`(2^32)-1`
* 字符串对象可能包括无效字节序列和接收到无效字节序列时基于实际的实现的反序列化。
    * 反序列化应该提供获取原始字节数组的功能，以便于应用程序可以决定如何处理这个对象。
...
MessagePack 允许应用程序通过扩展类型来定义应用程序的特殊类型。
扩展类型包括一个整数和一个字节数组。其中整数表示一种类型，字节数组表示数据。

应用程序可以通过`0` ～ `127`来保存应用程序的特殊类型信息。
MessagePack保留了`-1` ～ `-128`作为未来的扩展。
...
MessagePack序列化将MessagePack types 转换成如下所示的formats:
...
如果一个对象由多种输出format，那么应该将其序列化为最小字节数的数据format。


<a name="deserialization"/>
## Deserialization: format to type conversion

MessagePack反序列化MessagePack formats 为如下的types:
